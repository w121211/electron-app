...
const api = {
  ...
  pty: {
    ...
    onData: (callback: (sessionId: string, data: string) => void) => {
      ipcRenderer.on("pty:data", (_, sessionId, data) => callback(sessionId, data));
    },
    onExit: (callback: (sessionId: string, exitCode: number, signal?: number) => void) => {
      ipcRenderer.on("pty:exit", (_, sessionId, exitCode, signal) => callback(sessionId, exitCode, signal));
    },
    removeAllListeners: () => {
