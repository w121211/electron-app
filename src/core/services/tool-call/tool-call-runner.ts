...
export type ToolExecutionResult<TOOLS extends ToolSet> =
  | {
      ...
      toolCallsAwaitingConfirmation: Array<TypedToolCall<TOOLS>>;
    }
  | {
      status: "completed";
      executed: Array<TypedToolResult<TOOLS>>;
      failed?: Array<TypedToolError<TOOLS>>;
    };
...
export class ToolCallRunner<TOOLS extends ToolSet> {
  ...
  async execute(
    ...
    const executed: Array<TypedToolResult<TOOLS>> = [];
    const failed: Array<TypedToolError<TOOLS>> = [];
  ...
  private async initializeToolCallStatuses(
    ...
    confirmations: Array<ToolCallConfirmation>,
    alwaysAllowRules: Array<ToolAlwaysAllowRule>,
  ): Promise<Array<ToolCallWithStatus<TOOLS>>> {
    return Promise.all(
  ...
  private async executeToolDirectly(
    toolCall: TypedToolCall<TOOLS>,
    context: ToolExecutionContext,
  ): Promise<TypedToolResult<TOOLS> | TypedToolError<TOOLS>> {
