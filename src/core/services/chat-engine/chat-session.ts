...
export class ChatSession<TOOLS extends ToolSet = ToolSet> {
  ...
  private toolCallsAwaitingConfirmation: Array<TypedToolCall<TOOLS>> = [];
    ...
    try {
      ...
      if ("status" in input) {
        ...
        const message: ChatMessage = {
          ...
          message: {
            role: "tool",
            content: input.executed as Array<ToolResultPart>,
          },
        };
      ...
      const toolCallsMap = new Map<string, TypedToolCall<TOOLS>>();
          ...
          case "tool-error":
            if (chunk.error instanceof ToolConfirmationRequiredError) {
              ...
              if (toolCall === undefined) {
                throw new Error(
                  `Tool call ${chunk.toolCallId} not found in map for confirmation`,
                );
              }
