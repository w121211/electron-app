<!-- src/renderer/src/components/file-explorer/RenameDialog.svelte -->
<script lang="ts">
  ...
  function handleSubmit(): void {
    ...
    if (newName && newName !== fileExplorerState.renameDialog.currentName) {
      fileExplorerService.handleRename(
        fileExplorerState.renameDialog.targetPath,
        newName,
      );
    } else {
      closeRenameDialog();
  ...
  function handleClickOutside(event: MouseEvent): void {
    if (
      fileExplorerState.renameDialog.isVisible &&
      !(event.target as Element).closest(".rename-dialog")
    ) {
      handleCancel();
    }
  }
  ...
  $effect(() => {
    console.log(
      "🎯 RenameDialog $effect triggered, isVisible:",
      fileExplorerState.renameDialog.isVisible,
      "currentName:",
      fileExplorerState.renameDialog.currentName,
    );
...
{#if fileExplorerState.renameDialog.isVisible}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
    <div
      class="rename-dialog bg-surface border-border w-80 rounded-lg border p-4 shadow-xl"
    >
      ...
      <div class="mb-4">
        ...
        <input
          ...
          class="text-foreground bg-input-background border-input-border focus:border-accent focus:ring-accent/50 w-full rounded border px-3 py-2 text-sm focus:ring-1 focus:outline-none"
          type="text"
      ...
      <div class="flex justify-end gap-2">
        ...
        <button
          ...
          disabled={!inputValue.trim() ||
            inputValue.trim() === fileExplorerState.renameDialog.currentName}
