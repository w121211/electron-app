...
<script lang="ts">
  ...
  let {
    ...
    refreshEntries: (options?: { selectId?: string }) => Promise<void>;
  } = $props();
  ...
  const enqueueAutoSave = async (
    ...
    try {
      const updatedPrompt = await promptClientService.updatePrompt(promptId, {
        content,
      });
  ...
  const handleLaunchPrompt = async (): Promise<void> => {
    ...
    try {
      ...
      await refreshEntries({ selectId: session.id });
    } catch (error) {
  ...
  <div class="flex flex-1 flex-col p-3">
    ...
    {:else}
      <textarea
        ...
        class="bg-surface text-foreground placeholder:text-muted h-full w-full resize-none px-3 py-2 text-sm leading-6 outline-none"
