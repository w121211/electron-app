...
import { parseAtCommands, type Token } from '../src/renderer/src/utils/at-command-parser';
...
function runDemo() {
  ...
  testCases.forEach((text, index) => {
    ...
    tokens.forEach((token: Token, tokenIndex) => {
      if (token.type === 'text') {
        ...
        console.log(`  [${tokenIndex}] AT-COMMAND: "${token.raw}" → file: "${token.filePath}"`);
      }
    });
  ...
  const textWithFiles = 'Update @src/components/App.svelte, @"build output/main.js", and @package.json';
