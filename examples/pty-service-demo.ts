...
async function runPtyServiceDemo() {
  ...
  ptyService.on("exit", (id: string, exitCode: number, signal?: number) => {
    console.log(`🔴 Session ${id} exited with code ${exitCode}, signal: ${signal}\n`);
  });
  ...
  console.log(`📈 Total active sessions: ${ptyService.getAllSessions().length}\n`);
  ...
  console.log(`✅ Resize ${sessionId}: ${resizeSuccess ? "SUCCESS" : "FAILED"}\n`);
  ...
  console.log(`❌ Write to invalid session: ${invalidWrite ? "SUCCESS" : "FAILED"}`);
  
  const invalidResize = ptyService.resize("invalid-session", { cols: 80, rows: 24 });
  console.log(`❌ Resize invalid session: ${invalidResize ? "SUCCESS" : "FAILED"}`);
  
  const invalidDestroy = ptyService.destroy("invalid-session");
  console.log(`❌ Destroy invalid session: ${invalidDestroy ? "SUCCESS" : "FAILED"}\n`);

  // Demo 10: Clean up individual session
  console.log("🧹 Cleaning up one session...");
  const destroySuccess = ptyService.destroy(session2);
  console.log(`✅ Destroyed ${session2}: ${destroySuccess ? "SUCCESS" : "FAILED"}`);
  ...
  console.log(`📈 Final session count: ${ptyService.getAllSessions().length}\n`);
